...
int loadCellA, loadCellB, loadCellC;
int loadCellAPin = A10;
int loadCellBPin = A11;
int loadCellCPin = A12;
String lc;
boolean checkLC = 0;
...
void loop()
{
  ...
  //Read Load Cells
  loadCellA = analogRead(loadCellAPin);
  loadCellB = analogRead(loadCellBPin);
  loadCellC = analogRead(loadCellCPin);
  lc = "w";
  lc = lc + String(loadCellA,DEC) + ",";
  lc = lc + String(loadCellB, DEC) + ",";
  lc = lc + String(loadCellC, DEC) + ",";
  Serial.println(lc);
  }
  count2 = count2+1;
...
//      Serial.println("QD Air Open");
  ...
  if (!sysMonPrev && sysMon)  //rising edge
  {
    sysMonPrev = 1;
  ...
  else if (sysMonPrev && !sysMon) {   //falling edge
    sysMonPrev = 0;
    if (!igniterState) {
    dispBattVoltage();
    }
  ...
  //Find rising,falling edge of igniter enable
  if (igMon && !igMonPrev) {  //rising edge
    igMonPrev = 1;
    if (!igniterState) {
    dispBattVoltage();
    }
  }
  else if (!igMon && igMonPrev)   { //falling edge
    igMonPrev = 0;
    if (!igniterState) {
    dispBattVoltage();
    }
